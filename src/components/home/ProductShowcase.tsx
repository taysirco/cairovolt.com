...
import { showcaseProducts, type ShowcaseProduct, type ShowcaseRatings } from '@/data/showcase-products';

interface ProductShowcaseProps {
  locale: string;
  // Aggregate ratings precomputed server-side (see src/app/[locale]/page.tsx).
  // Passing them as a prop keeps the full ~16KB productReviewsDb OUT of the
  // homepage client bundle — it used to ship to every visitor just to render
  // 8 star ratings.
  ratings?: ShowcaseRatings;
}

export default function ProductShowcase({ locale, ratings }: ProductShowcaseProps) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-5">
          {showcaseProducts.map((product) => {
            const rating = ratings?.[product.slug] ?? null;
