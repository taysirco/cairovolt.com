...
export default function ShareButtons({ slug, productName, price, locale = 'ar', compact = false }: ShareButtonsProps) {
    ...
    const btnBase = compact
        ...
        // Mobile: icon-only + tight padding so all share buttons fit one row;
        // labels return at sm+ (see hidden sm:inline on each label span).
        : 'inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 hover:scale-105 active:scale-95';

    return (
        <div className={`flex items-center gap-1.5 sm:gap-2 ${compact ? '' : 'flex-nowrap sm:flex-wrap'}`} dir={isArabic ? 'rtl' : 'ltr'}>
            {!compact && (
            ...
            >
                ...
                {!compact && (
                    <span className="hidden sm:inline">{copied ? (isArabic ? 'تم!' : 'Copied!') : (isArabic ? 'نسخ' : 'Copy')}</span>
                )}
            </button>
            ...
            >
                ...
                {!compact && <span className="hidden sm:inline">{isArabic ? 'واتساب' : 'WhatsApp'}</span>}
            </button>
            ...
            >
                ...
                {!compact && <span className="hidden sm:inline">{isArabic ? 'فيسبوك' : 'Facebook'}</span>}
            </button>
            ...
            {supportsNativeShare && (
                ...
                >
                    ...
                    {!compact && <span className="hidden sm:inline">{isArabic ? 'مشاركة' : 'Share'}</span>}
                </button>
            )}
        </div>
    );
