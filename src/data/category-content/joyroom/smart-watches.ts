import type { CategoryContent } from '../_types';
export const joyroom_smart_watches_content: CategoryContent = {
            brand: 'Joyroom',
            brandColor: 'red',
            categoryName: 'Smart Watches',
            metadata: {
                en: {
                    title: 'Joyroom Smartwatch Egypt | AMOLED and Bluetooth Calling',
                    description: 'Compare Joyroom smartwatches by display, Bluetooth calling, rated battery life, app compatibility, and water-resistance rating. Current price and CairoVolt warranty are listed per product.',
                    keywords: 'جوي روم smartwatch, جوي روم smart watch, جوي روم watch egypt, جوي روم FT3 pro, جوي روم FT5, smart watch egypt, fitness tracker egypt, جوي روم AMOLED watch, ساعة جوي روم, bluetooth calling watch',
                },
                ar: {
                    title: 'ساعة جوي روم الذكية | AMOLED ومكالمات بلوتوث',
                    description: 'قارن ساعات جوي روم حسب الشاشة ومكالمات البلوتوث وعمر البطارية المعلن وتوافق التطبيق وتصنيف مقاومة الماء. السعر الحالي وضمان كايرو فولت موضحان لكل منتج.',
                    keywords: 'ساعة جوي روم, ساعة ذكية جوي روم, ساعة ذكية مصر, جوي روم FT3, ساعة AMOLED جوي روم, ساعة رياضية ذكية, ساعة بلوتوث كولينج, سعر ساعة جوي روم, ساعة مكالمات بلوتوث',
                }
            },
            pageContent: {
                ar: {
                    title: 'ساعة جوي روم الذكية حسب الشاشة والاتصال',
                    subtitle: 'قارن AMOLED ومكالمات البلوتوث والبطارية حسب الموديل',
                    description: `
تختلف ساعات **جوي روم** في نوع الشاشة وإصدار البلوتوث والمكالمات وعمر البطارية المعلن وتصنيف مقاومة الماء. قارن FT3 Pro وFT5 وغيرهما حسب المواصفات التي تحتاجها، ولا تفترض أن كل الموديلات تحمل الميزات نفسها.
راجع السعر الحالي وضمان كايرو فولت في صفحة المنتج. موعد التوصيل تقديري حسب العنوان، وتسوق [شاحن جوي روم](/joyroom/wall-chargers) فقط إذا كان متوافقاً مع متطلبات الساعة.
    `,
                    qualityBadges: [
                        { type: 'originality', text: 'الإصدار ورقم الموديل موضحان في صفحة المنتج' },
                        { type: 'warranty', text: 'ضمان كايرو فولت حسب صفحة المنتج والسياسة' },
                        { type: 'expert_verified', text: 'Bluetooth وAMOLED وتصنيف الماء حسب الموديل' }
                    ],
                    buyingGuide: [
                        {
                            title: 'مقارنة الموديلات: FT3 Pro وFT5 وFC2',
                            content: `
| الميزة | FT3 Pro | FT5 | FC2 |
|--------|---------|-----|-----|
| **الشاشة** | AMOLED 1.43" | IPS 1.83" | AMOLED 1.32" |
| **البطارية المعلنة** | حتى 7 أيام | حتى 5 أيام | حتى 6 أيام |
| **مكالمات** | ✅ بلوتوث 5.3 | ✅ بلوتوث 5.3 | ✅ بلوتوث 5.0 |
| **مقاومة ماء** | IP68 | IP68 | IP67 |
| **الوزن** | 52 جرام | 48 جرام | 45 جرام |
| **السعر** | راجع صفحة المنتج | راجع صفحة المنتج | راجع صفحة المنتج |
**نصيحة كايرو فولت:** شاشة AMOLED تقدم أسوداً أعمق عادةً، بينما قد توفر شاشة IPS حجماً أكبر في بعض الموديلات. قارن السطوع والحجم في المواصفات الفعلية.
`
                        },
                        {
                            title: 'إزاي تعرف الساعة الذكية المقلدة من الأصلية؟',
                            content: `
1. **الموديل:** طابق رقم الموديل ونوع الشاشة والحجم مع معلومات الشركة المتاحة.
2. **التطبيق:** تحقق من اسم التطبيق وإصدارات iOS وAndroid المدعومة قبل الشراء.
3. **البطارية:** استخدم المدة المعلنة للموديل باعتبارها تقديراً يتغير مع الشاشة والمكالمات والقياس.
4. **العلبة:** استخدم وسيلة التحقق الرسمية المتاحة إن وُجدت؛ كود QR أو السيريال وحده لا يثبت أصالة الشركة المصنّعة.
5. **الفاتورة والضمان:** احتفظ بفاتورة كايرو فولت وراجع المدة والشروط في صفحة المنتج والسياسة.
`
                        },
                        {
                            title: 'المواصفات التقنية المهمة',
                            content: `
| التقنية | إيه معناها ليك؟ |
|---------|----------------|
| **Bluetooth 5.3** | إصدار الاتصال؛ الجودة والمدى والاستهلاك تعتمد أيضاً على الهاتف والبيئة والموديل |
| **AMOLED** | ألوان وأسود عميق، مع سطوع وحجم يختلفان حسب الموديل |
| **IP68** | مقاومة ماء وغبار ضمن شروط الشركة؛ لا تفترض صلاحيتها للسباحة أو الماء الساخن |
| **مستشعر PPG** | تقديرات للنبض مخصصة للمتابعة العامة وليست للتشخيص الطبي |
| **GPS متصل** | تتبع الجري عبر GPS الموبايل — ليس GPS مدمج |
`
                        }
                    ],
                    faq: [
                        {
                            question: 'هل ساعة جوي روم بتشحن الموبايل؟ (سؤال شائع)',
                            answer: 'لا، الساعة لا تشحن الموبايل. عرض الإشعارات والرد على المكالمات من المعصم قد يقلل مرات فتح الشاشة، لكن أثر ذلك على بطارية الهاتف يختلف ولا توجد نسبة ثابتة.'
                        },
                        {
                            question: 'هل الساعة آمنة على المعصم؟ وهل بتسبب حساسية؟',
                            answer: 'راجع مادة السوار ووزن الموديل في صفحة المنتج. إذا كانت بشرتك حساسة، نظف السوار وجففه بانتظام وتوقف عن استخدامه عند ظهور تهيج.'
                        },
                        {
                            question: 'إيه الفرق بين FT3 Pro و FT5؟ أختار أنهي؟',
                            answer: 'FT3 Pro يذكر شاشة AMOLED دائرية 1.43 بوصة ومدة بطارية معلنة حتى 7 أيام، بينما يذكر FT5 شاشة IPS مربعة 1.83 بوصة ومدة حتى 5 أيام. اختر حسب الشكل والحجم، وراجع السعر الحالي والمواصفات في صفحة المنتج.'
                        },
                        {
                            question: 'هل الساعة بتتوافق مع الايفون والاندرويد؟',
                            answer: 'التوافق وإصدار البلوتوث وميزات المكالمات والإشعارات واللغة تختلف حسب الموديل وإصدار التطبيق والهاتف. راجع صفحة المنتج وقائمة الأنظمة المدعومة في متجر التطبيق.'
                        },
                        {
                            question: 'الساعة بتسخن ولا لا؟',
                            answer: 'قد تصبح الساعة دافئة أثناء الشحن أو المكالمات الطويلة. استخدم الشاحن الموصى به، وانزع الساعة وتوقف عن استخدامها إذا ظهرت سخونة غير طبيعية.'
                        }
                        ,{
                            question: 'هل ساعة جوي روم بتقيس ضغط الدم والأكسجين؟',
                            answer: 'الموديلات الداعمة تعرض تقديرات SpO2 ومعدل النبض للمتابعة العامة فقط. الساعة ليست جهازاً طبياً ولا ينبغي استخدامها للتشخيص أو اتخاذ قرار علاجي.'
                        }
                        ,{
                            question: 'هل ساعة جوي روم بتشتغل بدون موبايل؟',
                            answer: 'جزئياً. بتعرض الوقت والتاريخ وبتسجل الخطوات والنبض بدون موبايل. لكن المكالمات والإشعارات والطقس محتاجة اتصال بلوتوث بالموبايل.'
                        }
                        ,{
                            question: 'كم سعر ساعة جوي روم في مصر؟',
                            answer: 'السعر الحالي يظهر في بطاقة وصفحة كل موديل وقد يتغير مع المخزون والعروض. قارن الشاشة والمكالمات والتطبيق والبطارية، ثم راجع ضمان كايرو فولت المكتوب للمنتج.'
                        }
                    ],
                    products: [
                        { name: 'جوي روم FT3 Pro AMOLED (مكالمات بلوتوث)', price: 1092, badge: 'AMOLED دائري' },
                        { name: 'جوي روم FT5 شاشة كبيرة 1.83"', price: 1092, badge: 'أكبر شاشة' }
                    ]
                },
                en: {
                    title: 'Joyroom Smartwatches by Display and Connectivity',
                    subtitle: 'Compare AMOLED, Bluetooth calling, app support, and rated battery life',
                    description: `
Joyroom smartwatch models vary in display type, Bluetooth version, calling features, rated battery life, and water-resistance rating. Compare FT3 Pro, FT5, and other models by the specifications you need rather than assuming the entire range is identical.
Check the current price and CairoVolt warranty on the product page. Delivery timing is estimated from the confirmed address. A serial number or warranty record alone does not prove manufacturer authenticity.
    `,
                    qualityBadges: [
                        { type: 'originality', text: 'Version and model number listed per product' },
                        { type: 'warranty', text: 'CairoVolt warranty per product and policy' },
                        { type: 'expert_verified', text: 'Bluetooth, AMOLED, and water rating listed by model' }
                    ],
                    buyingGuide: [
                        {
                            title: 'Model Comparison: FT3 Pro vs FT5 vs FC2',
                            content: `
| Feature | FT3 Pro | FT5 | FC2 |
|---------|---------|-----|-----|
| **Display** | AMOLED 1.43" | IPS 1.83" | AMOLED 1.32" |
| **Rated battery** | Up to 7 days | Up to 5 days | Up to 6 days |
| **Calling** | ✅ BT 5.3 | ✅ BT 5.3 | ✅ BT 5.0 |
| **Water** | IP68 | IP68 | IP67 |
| **Weight** | 52g | 48g | 45g |
| **Price** | See product page | See product page | See product page |
**CairoVolt Tip:** AMOLED generally offers deeper blacks, while IPS may be available in a larger size on some models. Compare listed brightness and dimensions for the exact watch.
`
                        },
                        {
                            title: 'How to Spot a Fake Smart Watch',
                            content: `
1. **Model:** Match the model number, display type, and dimensions with the manufacturer's available information.
2. **App:** Confirm the app name and supported iOS or Android versions before purchase.
3. **Battery:** Treat the model's rated duration as an estimate affected by display, calls, and sensor use.
4. **Box:** Use an available official verification method if provided. A QR code or serial alone does not prove manufacturer authenticity.
5. **Invoice and warranty:** Keep the CairoVolt invoice and review duration and terms on the product page and policy.
`
                        },
                        {
                            title: 'Key Technical Specs Explained',
                            content: `
| Technology | What It Means For You |
|------------|---------------------|
| **Bluetooth 5.3** | Connection version; quality, range, and power use also depend on phone, environment, and model |
| **AMOLED** | Deep blacks and vivid colors, with brightness and size varying by model |
| **IP68** | Water and dust resistance within manufacturer conditions; do not assume suitability for swimming or hot water |
| **PPG Sensor** | General wellness estimates; not for medical diagnosis |
| **Connected GPS** | Run tracking via phone GPS — no built-in GPS |
`
                        }
                    ],
                    faq: [
                        {
                            question: 'Does a Joyroom watch charge the phone? (common question)',
                            answer: 'No. The watch does not charge the phone. Showing notifications and answering calls from the wrist may reduce how often you open the screen, but the effect on phone battery varies and there is no fixed percentage.'
                        },
                        {
                            question: 'Is the watch safe on the wrist? Can it cause irritation?',
                            answer: 'Check the strap material and model weight on the product page. If your skin is sensitive, clean and dry the strap regularly and stop use if irritation appears.'
                        },
                        {
                            question: 'What is the difference between FT3 Pro and FT5? Which should I choose?',
                            answer: 'FT3 Pro lists a round 1.43-inch AMOLED display and published battery life up to 7 days, while FT5 lists a square 1.83-inch IPS display and up to 5 days. Choose by shape and size, and check the live price and specs on the product page.'
                        },
                        {
                            question: 'Does the watch work with iPhone and Android?',
                            answer: 'Compatibility, Bluetooth version, call features, notifications, and language differ by model, app version, and phone. Check the product page and supported OS list in the app store.'
                        },
                        {
                            question: 'Does the watch get warm?',
                            answer: 'The watch may become warm during charging or long calls. Use the recommended charger, and remove the watch and stop using it if unusual heat appears.'
                        },
                        {
                            question: 'Does a Joyroom watch measure blood pressure and oxygen?',
                            answer: 'Supported models show SpO2 and heart-rate estimates for general tracking only. The watch is not a medical device and should not be used for diagnosis or treatment decisions.'
                        },
                        {
                            question: 'Does a Joyroom watch work without a phone?',
                            answer: 'Partially. It can show time and date and record steps and heart rate without a phone. Calls, notifications, and weather need a Bluetooth connection to the phone.'
                        },
                        {
                            question: 'How much does a Joyroom watch cost in Egypt?',
                            answer: 'The live price is shown on each model card and page and can change with stock and promotions. Compare display, calls, app, and battery, then review the CairoVolt warranty written for the product.'
                        }
                    ],
                    products: [
                        { name: 'Joyroom FT3 Pro AMOLED (BT Calling)', price: 1092, badge: 'Round AMOLED' },
                        { name: 'Joyroom FT5 Big Screen 1.83"', price: 1092, badge: 'Biggest Display' }
                    ]
                }
            }
        };
