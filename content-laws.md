...
| 6 | 🎯 Targeting & Article-Type Strategy (ماذا نستهدف + الجسر الاستشاري + دائم الخضرة) | [`cairovolt-keyword-strategy.md`](cairovolt-keyword-strategy.md) | 🟠 قوي |
| 7 | 🏆 Dominance Playbook (GEO/استشهاد AI + ميزات SERP + سلطة Pillar + بطاقة تقييم ≥80) | [`cairovolt-dominance-playbook.md`](cairovolt-dominance-playbook.md) | 🟢 لمقالات الصدارة |
| 8 | 🛒 Product Integrity (سلامة المنتجات) | **هنا §8** | 🔴 لا يُكسر |
| 9 | 📝 Meta Description Bounds (حدود الوصف) | **هنا §9** | 🔴 لا يُكسر |
| 10 | 🔑 Keyword Source (مصدر الكلمات) | **هنا §10** | 🔴 لا يُكسر |
| 11 | 📅 Scheduled Publishing (النشر المجدول) | **هنا §11** | 🔴 لا يُكسر |
| 12 | 📖 Mandatory File Reading (قراءة الملفات) | **هنا §12** | 🔴 لا يُكسر |
...
3. **📏 الحد الأدنى:** 🔴 **1,500 كلمة لكل لغة** (الهدف 1,500-2,000 عربي + نفس العمق EN) — تحقق بأمر §16-A في `cairovolt-blog.md` قبل أي commit
...
| §3 Voice | أول 200 كلمة فيها فكاهة، عامية 85% → 70%، 🔴 **1,500+ كلمة/لغة** (مُتحقَّق بأمر §16-A)؟ | [`voice.md`](voice.md) |
...
| [`cairovolt-blog-roadmap.md`](cairovolt-blog-roadmap.md) | قائمة الـ 228 موضوع + المنشور (محتوى حي) |

---

## 🎯 الخلاصة

7 قوانين أصلية + 5 قوانين جديدة عابرة لكل أنواع المحتوى. الـ `audit:blog` يفرض القوانين 8-11 آلياً.

**قبل أي كتابة — اقرأ هذا الملف أولاً. قبل أي commit — راجع Checklist أعلاه. قبل أي push — `npm run audit:blog {slug}`.**

---

## §8 — 🛒 Product Integrity Law (قانون سلامة المنتجات)

### المبدأ

كل `relatedProducts` slug يجب أن يكون **ملف `.ts` موجود فعلياً** في `src/data/products/`. الـ slug الوهمي يعني قسم "المنتجات المذكورة" يظهر **فاضي** للقارئ — أسوأ من عدم وجوده.
...
|------|--------|
| العدد | 5-6 منتجات |
| الصحة | كل slug = ملف موجود في `src/data/products/{slug}.ts` |
| الملاءمة | اختر منتجات **ذات صلة بالموضوع** (شواحن لمقال شحن، سماعات لمقال صوتيات) |

### طريقة التحقق

```bash
# قبل كتابة relatedProducts — شوف المنتجات الموجودة فعلاً:
ls src/data/products/*.ts | sed 's|.*/||;s|\.ts||' | grep -v '^_'

# للبحث عن منتجات براند معين:
ls src/data/products/ | grep anker | grep charger
ls src/data/products/ | grep joyroom | grep cable
ls src/data/products/ | grep soundcore
```

> ❌ **ممنوع تأليف slug** لم تره في `ls`. الـ `audit:blog` يفشل بـ HARD ERROR لو أي slug غير موجود.

---

## §9 — 📝 Meta Description Bounds (قانون حدود الوصف)

### المبدأ

`metaDescription` هي الـ Search Snippet — أول ما يراه القارئ في جوجل. قصيرة جداً = جوجل يسحب نص عشوائي. طويلة جداً = تُقطع.

### القاعدة

| اللغة | الحد الأدنى | المثالي | الحد الأقصى |
|------|----------|----------|----------|
| AR `metaDescription` | **140 حرف** | 145-155 | 160 (تحذير) |
| EN `metaDescription` | **140 حرف** | 145-155 | 160 (تحذير) |

- `< 140` → 🔴 HARD ERROR — `audit:blog` يفشل
- `140-160` → ✅ نجاح
- `> 160` → ⚠️ تحذير (قد تُقطع في Google)

### لماذا هذا القانون؟

في مقال #102، EN `metaDescription` كانت **9 حروف فقط** (مقطوعة!) — والـ audit لم يكشفها. الآن الـ audit يفرض هذا الحد.

---

## §10 — 🔑 Keyword Source Law (قانون مصدر الكلمات المفتاحية)

### المبدأ

الكلمات المفتاحية يجب أن تأتي من **بيانات بحث حقيقية** — ليس من تخمين الأيجنت.

### القاعدة

```bash
# إلزامي قبل كتابة keywords في أي مقال:
npm run kw:blog -- --slug={slug}
```

| المصدر | الحالة |
|--------|--------|
| ناتج `npm run kw:blog` | ✅ مقبول |
| كلمات يدوية بدون تشغيل الأمر | ❌ ممنوع |
| ناتج الأمر + تعديل يدوي طفيف | ✅ مقبول (لكن الأساس من الأمر) |

> الأمر يستخرج من 3 مصادر: `complete-keywords.csv` + `master-relevant.csv` + `article-final.json` + يولّد كلمات ذكية للمواضيع غير المغطاة.

---

## §11 — 📅 Scheduled Publishing Law (قانون النشر المجدول)

### المبدأ

كل مقال يُنشر بمعدل **مقال/يوم** بوقت عشوائي — حتى لو كُتبوا كلهم اليوم.

### القاعدة

```bash
# إلزامي قبل كتابة publishDate:
npm run next-date --json
```

| المصدر | الحالة |
|--------|--------|
| ناتج `npm run next-date` | ✅ مقبول |
| تاريخ اليوم يدوياً | ❌ ممنوع (يكسر الجدولة) |
| تاريخ ثابت مستقبلي | ❌ ممنوع (قد يتعارض مع مقال آخر) |

---

## §12 — 📖 Mandatory File Reading Law (قانون قراءة الملفات الإلزامية)

### المبدأ

الأيجنت **يجب** أن يقرأ الملفات المرجعية فعلياً باستخدام `view_file` — **لا يعتمد على الذاكرة أو السياق السابق**. القواعد تتغير، والسياق يُقطع، والذاكرة تكذب.

### القاعدة

| الترتيب | الملف | المطلوب |
|--------|------|--------|
| 1 | [`voice.md`](voice.md) | `view_file` إلزامي قبل الكتابة |
| 2 | [`content-laws.md`](content-laws.md) | `view_file` إلزامي قبل الكتابة |
| 3 | [`cairovolt-keyword-strategy.md`](cairovolt-keyword-strategy.md) | `view_file` إلزامي لتحديد الزاوية |
| 4 | [`cairovolt-blog-roadmap.md`](cairovolt-blog-roadmap.md) | `view_file` إلزامي لتحديد الموضوع + relatedArticles |

> ❌ **"أعرف القواعد"** ليست إجابة مقبولة — اقرأ الملف فعلياً في كل جلسة.
