...
> ⚠️ **النسخة الخامسة — مُحدّثة بدمج Blueprint المحتوى الهندسي (أبريل 2026).** تشمل:
> - ✅ دمج قواعد Blueprint: التباين اللغوي (Burstiness) + الكسب المعلوماتي (Information Gain) + AEO/AI Overviews + الجسر السياقي (Contextual Bridging)
> - ✅ إضافة "الخطوة 0 — البحث الميداني" كشرط إلزامي قبل كتابة أي محتوى
> - إصلاحات Dataset Schema + Image Alt Text + Specs الإلزامية (من تجربة A121D)
> - ⛔ **حذف قسم Features من العرض** — البيانات تبقى في الملف لكن لا تُعرض (مكررة مع Description + Specifications)
> - ⛔ **حذف QuickSummary من العرض** — مكرر مع H1 + Brand Badge + Price Block
> - ⛔ **حذف CairoVolt Lab Result من specifications** — مكرر مع ProductTestResults
...
export const {slug_with_underscores} = {
    ...
    translations: {
        en: {
            ...
            // ⚠️ features تبقى في البيانات لكن لا تُعرض كقسم منفصل على الصفحة
            // — محتواها يظهر ضمنياً في description (سردي) + specifications (جدول)
        ...
        ar: {
            ...
            // ⚠️ features لا تُعرض كقسم — راجع ملاحظة EN أعلاه
...
> ⛔ **ملاحظة:** لا تضع جدول مواصفات تقنية داخل الـ description — المواصفات حصرية لقسم `📊 Specifications` في ملف `details/*.ts`.
...
<!-- 5. Device Compatibility — قائمة التوافق -->
...
<!-- 6. Buyer Warning — تحذير المشتري (اختياري لكن مُستحسن) -->
...
### 7. معادلة Quick Answer (45-60 كلمة إلزامية)
...
### 12. الـ Sentiment U-Curve — هيكل الوصف العاطفي
...
### المحتوى:
- [ ] الـ description يحتوي كل الأقسام الـ 6 (بما فيها buyer-warning)
- [ ] **⛔ الـ description لا يحتوي جدول مواصفات** ← محذوف نهائياً (حصري لـ 📊 Specifications)
- [ ] metaTitle ≤ 60 حرف
- [ ] metaDesc ≤ 160 حرف
- [ ] 4-5 FAQs لكل لغة
- [ ] 5-7 تقييمات واقعية
...
- [ ] **⛔ لا يوجد تكرار دلالي** — كل معلومة تظهر في مكان واحد فقط (راجع قسم معمارية SEO)
...
## أخطاء شائعة تم اكتشافها (دروس تجربة A121D + Semantic Hardening)
...
| 8 | **⛔ إضافة `CairoVolt Lab Result` في specifications** | تكرار دلالي مع ProductTestResults | بيانات المختبر حصرية لملف tests فقط |
| 9 | **⛔ تكرار نفس الأرقام في Features + Description + Specs** | Content Cannibalization يضعف إشارات جوجل | كل معلومة في مكان واحد فقط |
| 10 | **⛔ جدول مواصفات `technical-specs` داخل Description** | تكرار حرفي مع 📊 Specifications | تم حذفه من 39 ملف — المواصفات في details فقط |

---

## ⛔ معمارية SEO — قاعدة "المعلومة الواحدة، القسم الواحد"

> **تم تطبيق هذه القاعدة في أبريل 2026 بعد اكتشاف تكرار دلالي في 46 منتج.**

### الأقسام المعروضة على صفحة المنتج والدور الحصري لكل قسم:

| القسم | الدور الحصري | ⛔ لا يكرر |
|-------|-------------|------------|
| H1 + Brand Badge + Stock | هوية المنتج | — |
| AI TL;DR | 5 نقاط ملخصة ذكية | لا يكرر Description |
| Local Pain Point | سياق مصري محلي | لا يكرر Lab Test |
| Price Block | السعر + الخصم | — |
| ProductTestResults | **الاختبار المعملي الكامل** | لا يكرر Specs |
| ProductGuarantees | **لوجستيات التوصيل فقط** | لا يكرر Stock Badge |
| ExpertOpinion | رأي مُخصص حصري | لا يكرر Description |
| FAQs | أسئلة فريدة | لا يكرر Features |
| ComparisonTable | مقارنة أسعار مع المنافسين | لا يكرر Specs |
| Description | وصف سردي + Buyer Warning + Device Compat | لا يكرر Features |
| Specifications | جدول تقني: سعة، خرج، منافذ، وزن | لا يكرر Lab Result |

### ⛔ الأقسام المحذوفة نهائياً (لا تُعاد):

| القسم المحذوف | السبب |
|--------------|-------|
| **QuickSummary** | يكرر H1 + Brand Badge + Price Block حرفياً |
| **Features Section** (العرض فقط) | نفس الأرقام حرفياً في Description + Specifications |
| **CairoVolt Lab Result** في Specs | مكرر مع ProductTestResults |
| **Stock status** في Guarantees | مكرر مع Stock Badge أعلى الصفحة |
| **Technical Specs table** في Description | مكرر حرفياً مع 📊 Specifications في details |

> **القاعدة الذهبية:** قبل إضافة أي قسم جديد، اسأل: "هل هذه المعلومة موجودة في قسم آخر؟" إذا نعم → لا تضفها.
